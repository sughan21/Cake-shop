...
function saveCatalog(catalog) {
  ...
  // Auto-sync catalog changes (stock, prices, new items) to Supabase catalog_items
  try {
    if (typeof syncCatalogToSupabase === 'function') {
      syncCatalogToSupabase(catalog);
    }
  } catch (e) {
    console.warn('Catalog auto-sync notice:', e);
  }
}
...
function finalizeCurrentOrder(downloadPdf = false) {
  ...
  try {
    syncOrderToSupabase(saleRecord);
  } catch (e) {
...
function getApiKeyConfig() {
  try {
    ...
    const defaultConfig = {
      provider: env.SUPABASE_URL ? 'supabase' : 'local',
      publishableKey: env.SUPABASE_PUBLISHABLE_KEY || env.SUPABASE_ANON_KEY || '',
      secretKey: '', // Do NOT store secret service role keys in client storage or source code
      apiKey: env.SUPABASE_PUBLISHABLE_KEY || env.RESEND_API_KEY || '',
...
function validateEnvConfig() {
  ...
  if (provider === 'supabase') {
    const url = cfg.endpoint || env.SUPABASE_URL;
    const key = cfg.publishableKey || cfg.apiKey || env.SUPABASE_PUBLISHABLE_KEY || env.SUPABASE_ANON_KEY;
    if (!url) issues.push('SUPABASE_URL is not set.');
    if (!key) issues.push('SUPABASE_PUBLISHABLE_KEY / SUPABASE_ANON_KEY is not set.');
  }
...
function updateCloudSyncBadge(status) {
  ...
  btn.classList.remove('syncing', 'error', 'local', 'connected');
...
function checkAndPromptSupabaseKey() {
  const { url, key } = getEffectiveSupabaseCredentials();
  const banner = document.getElementById('supabaseSetupNoticeBanner');
  if (!key) {
...
function initSupabaseDatabase() {
  try {
    const { url, key } = getEffectiveSupabaseCredentials();
    
    checkAndPromptSupabaseKey();
    ...
    console.warn('Supabase DB initialization warning:', e);
    supabaseClient = null;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initSupabaseDatabase);
} else {
...
async function syncCatalogToSupabase(catalog) {
  ...
  const payloads = catalog.map((item, idx) => ({
    id: String(item.id || ('prod_' + (idx + 1) + '_' + item.name.toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 10))),
    name: item.name,
    category: item.category || 'Cakes & More',
    price: Number(item.price || 0),
    stock: Number(item.stock != null ? item.stock : 100),
  ...
  try {
    if (supabaseClient) {
      const { error } = await supabaseClient.from('catalog_items').upsert(payloads, { onConflict: 'id' });
      if (!error) {
        console.log('✅ Catalog items successfully synced to Supabase catalog_items table');
        return;
      }
    }

    // Direct REST API fallback
    const res = await fetch(`${url.replace(/\/$/, '')}/rest/v1/catalog_items`, {
      method: 'POST',
      headers: {
        'apikey': key,
    ...
    if (res.ok) {
      console.log('✅ Catalog items synced to Supabase via REST');
    }
  } catch (err) {
    console.warn('Supabase catalog auto-sync note:', err);
  }
}
...
async function fetchCatalogFromSupabase() {
  ...
  try {
    let items = [];
    if (supabaseClient) {
      const { data, error } = await supabaseClient.from('catalog_items').select('*');
      if (!error && Array.isArray(data) && data.length > 0) items = data;
    }
    if (items.length === 0) {
      const res = await fetch(`${url.replace(/\/$/, '')}/rest/v1/catalog_items?select=*`, {
        method: 'GET',
    ...
    if (items.length > 0) {
      ...
      const localMap = new Map(localCatalog.map(i => [(i.name || '').toLowerCase(), i]));
      let updated = false;

      items.forEach(cloudItem => {
        const existing = localMap.get((cloudItem.name || '').toLowerCase());
        if (existing) {
          if (existing.price !== Number(cloudItem.price) || existing.stock !== Number(cloudItem.stock)) {
            existing.price = Number(cloudItem.price);
            existing.stock = Number(cloudItem.stock);
            updated = true;
          }
        } else {
          localCatalog.push({
            id: cloudItem.id,
            name: cloudItem.name,
            category: cloudItem.category || 'Cakes & More',
            price: Number(cloudItem.price),
            stock: Number(cloudItem.stock != null ? cloudItem.stock : 100),
            image: cloudItem.image_url || ''
          });
          updated = true;
        }
      });
      ...
      if (updated) {
        localStorage.setItem('sugarCubesCatalog', JSON.stringify(localCatalog));
        renderPreloadedCatalog();
        console.log('✅ Frontend menu updated with latest items from Supabase');
      }
    }
  } catch (err) {
    console.warn('Supabase catalog fetch note:', err);
  }
}

async function syncDailySalesAuditToSupabase() {
  const { url, key } = getEffectiveSupabaseCredentials();
  if (!url || !key) return;
  ...
  allSales.forEach(sale => {
    const dateKey = sale.isoDate || (sale.date ? new Date(sale.date).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10));
    if (!auditMap[dateKey]) {
      auditMap[dateKey] = {
        audit_date: dateKey,
        total_revenue: 0,
    ...
    auditMap[dateKey].total_revenue += amt;
    auditMap[dateKey].total_orders += 1;

    const pMode = (sale.paymentMode || sale.payment_method || 'Cash').toLowerCase();
    if (pMode.includes('cash')) {
      auditMap[dateKey].cash_collected += amt;
    } else if (pMode.includes('upi')) {
      auditMap[dateKey].upi_collected += amt;
    } else if (pMode.includes('card')) {
      auditMap[dateKey].card_collected += amt;
    } else {
      auditMap[dateKey].cash_collected += amt;
    }
  });

  const payloads = Object.values(auditMap);

  try {
    if (supabaseClient) {
      const { error } = await supabaseClient
        .from('daily_sales_audits')
        .upsert(payloads, { onConflict: 'audit_date' });
      if (!error) {
        console.log('✅ Daily sales audits successfully synced to Supabase');
        return;
      }
    }

    const res = await fetch(`${url.replace(/\/$/, '')}/rest/v1/daily_sales_audits`, {
      method: 'POST',
      headers: {
        'apikey': key,
        'Authorization': `Bearer ${key}`,
    ...
    if (res.ok) {
      console.log('✅ Daily sales audits synced to Supabase via REST API');
    }
  } catch (err) {
    console.warn('Supabase daily sales audit sync note:', err);
  }
}
...
async function fetchRegisteredUsersFromSupabase() {
  const { url, key } = getEffectiveSupabaseCredentials();
  if (!url || !key) return;
  ...
  try {
    ...
    if (supabaseClient) {
      ...
      if (!error && Array.isArray(data) && data.length > 0) cloudUsers = data;
    }
    if (cloudUsers.length === 0) {
      ...
      if (res.ok) {
        const data = await res.json();
        if (Array.isArray(data) && data.length > 0) cloudUsers = data;
      }
    }

    if (cloudUsers.length > 0) {
      const localUsers = getUsersList();
      const localMap = new Map(localUsers.map(u => [(u.email || u.id).toLowerCase(), u]));
      let updated = false;

      cloudUsers.forEach(cu => {
        const keyVal = (cu.email || cu.id || '').toLowerCase();
        if (keyVal && !localMap.has(keyVal)) {
          localUsers.push({
            id: cu.id || cu.email,
            email: cu.email || cu.id,
            password: cu.password || 'admin123',
            name: cu.name || 'Staff User',
            role: cu.role || 'Cashier',
            createdAt: cu.created_at || new Date().toISOString()
          });
          localMap.set(keyVal, true);
          updated = true;
        }
      });
      ...
      if (updated) {
        ...
        console.log('✅ Local staff accounts updated from Supabase registered_users table');
      }
    }
  } catch (err) {
    console.warn('Supabase registered users fetch note:', err);
  }
}

async function syncOrderToSupabase(saleRecord) {
  ...
  const payload = {
    ...
    payment_method: saleRecord.paymentMode || 'Cash',
    items: Array.isArray(saleRecord.items) ? saleRecord.items : [],
    subtotal: Number(saleRecord.subtotal || 0),
    gst_amount: Number(saleRecord.gst || 0),
    discount_amount: Number(saleRecord.discount || 0),
    grand_total: Number(saleRecord.grandTotal || saleRecord.amount || 0),
    cashier_name: (activeUser && activeUser.name) ? activeUser.name : 'Front Cashier',
    status: 'Completed'
  };

  try {
    if (supabaseClient) {
      const { data, error } = await supabaseClient.from('orders').upsert([payload], { onConflict: 'ticket_number' });
      if (error) {
        console.warn('⚠️ Supabase client insert note:', error.message || error);
        await directRestInsertOrder(url, key, payload);
      } else {
        console.log('✅ Order successfully saved to Supabase orders table:', payload.ticket_number);
    ...
    console.warn('⚠️ Order saved locally, Supabase background sync failed:', err);
  }
}

async function directRestInsertOrder(url, key, payload) {
  try {
    ...
    if (res.ok) {
      console.log('✅ Order synced to Supabase orders table via REST API:', payload.ticket_number);
      updateCloudSyncBadge('synced');
    } else {
      const errText = await res.text();
      console.warn('⚠️ REST sync returned:', res.status, errText);
    }
  } catch (e) {
    console.warn('⚠️ Direct REST network note:', e);
  }
}
...
async function syncOrdersWithSupabase(showNotifications = false) {
  ...
  if (!url || !key) {
    if (showNotifications) {
      showToast('ℹ️ Please configure your Supabase Project URL and Anon Key in Settings.');
      openApiKeyModal();
    }
    return;
  }
  ...
  try {
    ...
    if (supabaseClient) {
      try {
        ...
        console.warn('supabaseClient query warning:', clientErr);
      }
    }
    ...
    if (cloudOrders.length === 0) {
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(restData)) cloudOrders = restData;
        }
      } catch (err) {
        console.warn('Direct REST fetch failed:', err);
      }
    }

    const localOrders = getRecordedSales();
    const localTicketSet = new Set(localOrders.map(o => o.ticketNumber));

    // Convert Supabase rows to app format & merge into local orders
    ...
    cloudOrders.forEach(row => {
      ...
      if (!localTicketSet.has(ticketNum)) {
        ...
        localOrders.push(mappedRecord);
        localTicketSet.add(ticketNum);
        newFromCloudCount++;
      }
    });

    // Upload local orders that are missing in Supabase
    const cloudTicketSet = new Set(cloudOrders.map(o => o.ticket_number || o.ticketNumber));
    let pushedToCloudCount = 0;
    for (const localSale of localOrders) {
      if (localSale.ticketNumber && !cloudTicketSet.has(localSale.ticketNumber)) {
        await syncOrderToSupabase(localSale);
        pushedToCloudCount++;
      }
    }
    ...
    syncDailySalesAuditToSupabase();
    if (showNotifications) {
      showToast(`☁️ Cloud Sync complete! (${cloudOrders.length} in Cloud, +${pushedToCloudCount} uploaded, +${newFromCloudCount} downloaded)`);
    }
  } catch (err) {
    console.warn('Error during Supabase sync:', err);
    updateCloudSyncBadge('error');
    if (showNotifications) {
      showToast('⚠️ Cloud Sync failed. Check your Supabase API credentials.');
    }
  } finally {
...
async function testSupabaseConnection() {
  ...
  const alertEl = document.getElementById('apiConfigAlertBanner');
  ...
  if (!url) {
    if (alertEl) {
      ...
      alertEl.innerHTML = '❌ Please enter your Supabase Project URL (e.g. <code>https://your-id.supabase.co</code>)';
    }
    return;
  }
  ...
  if (!key) {
    if (alertEl) {
      ...
      alertEl.style.color = '#991b1b';
      alertEl.style.border = '1px solid #fecdd3';
      alertEl.innerHTML = '❌ Please enter your Supabase <strong>anon public key</strong>.';
    }
    return;
  }
  ...
  if (alertEl) {
    ...
    alertEl.innerHTML = `⏳ Testing connection to Supabase <code>${escapeHtml(url)}</code>...`;
  }

  try {
    const res = await fetch(`${url.replace(/\/$/, '')}/rest/v1/orders?select=id,ticket_number&limit=1`, {
      method: 'GET',
      headers: {
        'apikey': key,
        'Authorization': `Bearer ${key}`
    ...
    if (res.ok) {
      ...
      try {
        ...
        if (jsonErr.message) hint = ` (${jsonErr.message})`;
        if (jsonErr.hint) hint += ` - ${jsonErr.hint}`;
      } catch (e) {
    ...
    if (alertEl) {
      ...
      alertEl.style.color = '#991b1b';
      alertEl.style.border = '1px solid #fecdd3';
      alertEl.innerHTML = `❌ <strong>Network error:</strong> Unable to connect to <code>${escapeHtml(url)}</code>. Check internet connection.`;
    }
  }
}
...
async function sendResendEmailNotification(toEmail, subject, htmlContent) {
  const cfg = getApiKeyConfig();
  const env = getRuntimeEnv();
  const apiKey = cfg.apiKey || env.RESEND_API_KEY || '';
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: env.RESEND_FROM_EMAIL || 'Sugar Cubes POS <onboarding@resend.dev>',
        to: [toEmail],
        subject: subject,
        html: htmlContent
      })
...
function openApiKeyModal() {
  ...
  const keyInput = document.getElementById('cfgApiKey');
  const secretKeyInput = document.getElementById('cfgSecretKey');
  const endpointInput = document.getElementById('cfgApiEndpoint');
  const alertEl = document.getElementById('apiConfigAlertBanner');
  if (alertEl) alertEl.style.display = 'none';
  
  if (providerSel) providerSel.value = cfg.provider || (env.SUPABASE_URL ? 'supabase' : 'local');
  if (keyInput) keyInput.value = cfg.publishableKey || cfg.apiKey || env.SUPABASE_PUBLISHABLE_KEY || env.SUPABASE_ANON_KEY || '';
  if (secretKeyInput) secretKeyInput.value = cfg.secretKey || '';
  if (endpointInput) endpointInput.value = cfg.endpoint || env.SUPABASE_URL || '';
...
function handleApiProviderChange() {
  ...
  const provider = providerSelect ? providerSelect.value : 'supabase';
  const keyWrap = document.getElementById('apiKeyInputWrap');
  const secretWrap = document.getElementById('apiSecretKeyInputWrap');
  ...
  if (provider === 'local') {
    ...
    if (secretWrap) secretWrap.style.display = provider === 'supabase' ? 'block' : 'none';
...
function saveApiKeySettings() {
  ...
  const publishableKey = keyInput ? keyInput.value.trim() : '';
  ...
  initSupabaseDatabase();
  showToast('⚡ Supabase settings saved! Syncing cloud database...');
  closeApiKeyModal();
